...
var HttpNpmDataStore = function(){
    ...
    this.getRegistryConfig = function(){
        if (config.http.listRegistry.length > 1){
            for(let i = 0; i < config.http.listRegistry.length; i++) {
                let item = config.http.listRegistry[i];
                if (item.timeError){
                    let now = new Date().getTime();
                    if (now > (item.timeError + config.http.registryCircuitBreakerTimeOut)){
                        item.timeError = null;
                        return item;
                    }
                }
            ...
            let item = config.http.listRegistry[0];
            if (item.timeError){
                if ((item.timeError + config.http.registryCircuitBreakerTimeOut) > new Date().getTime()){
                    item.timeError = null;
                    return item;
                }
            }
    ...
    this.getModuleManifest = function(name, version, callBack){
        ...
        function request(){
            ...
            if (registryConfig){
                ...
                self.request(url, headers, function(err, data){
                    if (err){
                        if (err instanceof Error){
                            registryConfig.timeError = new Date().getTime();
                            request();
    ...
    this.request = function(url, headers, callBack){
        ...
        option.timeout = config.requestTimeout;
        option.method = "GET";
