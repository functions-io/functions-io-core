...
var InvokeFactory = function(p_moduleFactory){
    ...
    this.envObj = (function(){
        try {
            var envPath = path.join(path.dirname(require.main.filename), "env.json");
    ...
    this.getEnv = function(moduleName, moduleVersion, key){
        var item;
        var value;
        if (this.envObj){
            item = this.envObj[moduleName + ":" + moduleVersion];

            if (!item){
                item = this.envObj[moduleName];
            }
    ...
    this.buildContext = function(context, moduleName, moduleVersion){
        ...
        if (context.getEnv === undefined){
            context.getEnv = function(key){
                return self.invokeAsync(moduleName, moduleVersion, key);
            };
        }
