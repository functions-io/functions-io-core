...
var FileExtension = function(factory){
    ...
    this.addFunctionManagerFromFolder = function(basePATH, trackFileChanged, callBack){
    ...
    this.addFunctionManagerFromFolderModules = function(trackFileChanged, callBack){
        folder.getListFilesModules(function(err, files){
            if (err){
                callBack(err);
            }
            else{
    ...
    this.addFunctionManagerFromFiles = function(files, callBack){
        ...
        files.map(function(item){
            if (item.status === undefined || item.status === "ADD" || item.status === "CHANGED"){
                ...
                self.addFunctionManagerFromPackageFile(item, function(err, key){
                    var newItemResult = {};
                    if (err){
    ...
    this.addFunctionManagerFromPackageFile = function(item, callBack){
