...
var ModuleFactory = function(){
    ...
    this.memoryRegistry = new MemoryRegistry(this.moduleCache);
    ...
    this.requireAsync = function(moduleName, moduleVersion){
        return new Promise(function(resolve, reject){
            try {
                var resultObj = self.moduleCache.getObjectInCache(moduleName, moduleVersion);
                if (resultObj){
                    resolve(resultObj);
                }
                else{
                    self.memoryRegistry.getDataStore(moduleName, moduleVersion, function(err, buffer, versionTarget){
                        try {
                            if (err){
                                ...
                                resultObj = self.requireSync(moduleName, versionTarget);
    ...
    this.requireSync = function(moduleName, moduleVersion, fileName, parentManifest){
        ...
        //cache
        var resultObj = this.moduleCache.getObjectInCache(moduleName, versionTarget, fileName);
        if (resultObj){
            return resultObj;
        }
    ...
    this.getManifestSync = function(moduleName, moduleVersion){
        return this.moduleCache.getManifestInCache(moduleName, moduleVersion);
    };
