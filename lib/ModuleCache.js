...
var ModuleCache = function(){
    ...
    this.getManifestInCache = function(moduleName, moduleVersion){
        ...
        if (mainCacheItem){
            ...
            var mainLocalCacheItem = this.cacheLocalObjects.get(moduleName);
            if (mainLocalCacheItem){
                return mainLocalCacheItem.moduleManifestObj || null;
            }
            else{
                return null;
            }
    ...
    this.loadLocalModuleInCache = function(modulePath, isLazy){
        try {
            var packagePath = path.join(modulePath, "package.json");
            if (fs.existsSync(packagePath)){
                let obj = null;
                let manifestObj = {};
                manifestObj.packageObj = require(packagePath);
                ...
                try {
                    manifestObj.input = require(path.join(modulePath, "input.json"));
                }
                catch (error) {
                    manifestObj.input = null;
                }

                try {
                    manifestObj.output = require(path.join(modulePath, "output.json"));
                }
                catch (error) {
                    manifestObj.output = null;
                }
                
                manifestObj.versions = [manifestObj.packageObj.version];

                log.debug(__filename, "loadLocalModuleInCache", {moduleName:manifestObj.packageObj.name, moduleVersion:manifestObj.packageObj.version});

                return self.addLocalObjectInCache(obj, manifestObj);
            }
        }
        catch (errTryLocalRequire) {
            log.error(__filename, "loadLocalModuleInCache", {message:errTryLocalRequire.message});
            return null;
        }
    };
                        ...
                        try {
                            if (fs.statSync(filePath).isDirectory()){
                                if (itemFile.substring(0,1) === "@"){ //subfolder
                                    ...
                                    this.loadLocalModuleInCache(filePath, isLazy);
                                }
                            }                    
                        }
                        catch (errTry2) {
