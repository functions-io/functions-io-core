...
const basePathNodeModules = path.join(path.dirname(require.main.filename), "node_modules");
...
var ModuleCache = function(){
    ...
    this.searchAndLoadLocalModulesInNodeModulesFolder = function(isLazy){
        try {
            let listFiles = fs.readdirSync(basePathNodeModules);
            for (var i = 0; i < listFiles.length; i++){
                if (listFiles[i].substring(0,1) !== "."){
                    let filePath = path.join(basePathNodeModules, listFiles[i]);
                    try {
    ...
    this.loadLocalObjectInCache = function(modulePath, isLazy){
        ...
        try {
            ...
            log.error(__filename, "getLocalObjectInCacheAndLoadIfNotExist", {message:errTryLocalRequire.message});
            return null;
        }
    };
